...
app.post('/v1/append-workspace-document-packet', async function (request, response) {
  const organizationId = request.header('X-Org-Id') ?? null;
  const workspaceId = request.header('X-Workspace-Id') ?? null;
  ...
  if (!document) {
    response.sendStatus(404).json({ error: `No document found with id ${docId}.` });
    return;
  }

  const filepath = `fragments/${document.uid}/fragment_${order}-${uuidv4()}.cjson`;
  await exportFile(filepath, JSON.stringify(packet), 'application/json', true);
...
app.get('/v1/:documentId/conclude-workspace-document-sync', async function (request, response) {
  ...
  await Document.updateQuick(documentId, { readyForCache: true, cacheFilepath: null })
